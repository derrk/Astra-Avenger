class Gurilla extends AI {
    ...
    moveDown() {
        this.obj.setVelocity(0, .4 * this.scene.plySpd);
        // Check if we should change states
        if ((this.scene.player.y - this.obj.y) < 450) {
    ...
    strafe() {
        ...
        if (!this.strafing) {
            this.strafing = true;
            // Go right
            this.obj.setVelocity(.4 * this.scene.plySpd, 0);
        }
        ...
        if (this.now() > this.lastShot + this.shotTimeout) {
            this.scene.createBullet(this.obj.x, this.obj.y + 70, true);
